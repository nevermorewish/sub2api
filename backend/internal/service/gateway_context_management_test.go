...
func TestComputeFinalAnthropicBeta_OAuthMimic_Haiku_MatchesCapturedToolHeader(t *testing.T) {
	s := newTestGatewayServiceForBeta(false)
	...
	require.True(t, anthropicBetaTokensContains(final, claude.BetaRedactThinking))
	require.True(t, anthropicBetaTokensContains(final, claude.BetaContextManagement))
...
func TestBuildUpstreamRequest_OAuthMimicHaiku_PreservesCapturedContextManagementEndToEnd(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
	...
	// haiku + mimic CC → final beta = verify-haiku45-tool-20260530 抓包 header（含 context-management）→
	// body 必须保留。
	body := []byte(`{"model":"claude-haiku-4-5","context_management":{"edits":[{"type":"clear_thinking_20251015"}]},"messages":[]}`)
	svc := &GatewayService{cfg: &config.Config{}}
	...
	require.Equal(t, claude.HaikuBetaHeader, outBeta)
	require.True(t, anthropicBetaTokensContains(outBeta, claude.BetaContextManagement),
		"对齐 verify-haiku45-tool-20260530：outgoing anthropic-beta header 带 context-management beta")
}
