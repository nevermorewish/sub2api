export default {
    accounts: {
      ...
      autoMonitor: '半小时自动监测',
      autoMonitorHint: '每 30 分钟按“测试连接”的方式向所有非禁用账号发送 hi；成功后恢复账号状态并重新开启调度。',
      autoMonitorEnabled: '已开启半小时账号自动监测',
      autoMonitorDisabled: '已关闭半小时账号自动监测',
      autoMonitorUpdateFailed: '更新账号自动监测失败',
      autoMonitorRunNow: '立即执行账号监测',
      autoMonitorRunSuccess: '账号监测完成：共 {total} 个，成功 {succeeded} 个，失败 {failed} 个',
      autoMonitorRunFailed: '手动执行账号监测失败',
      ...
      glmMultimodal: '支持 GLM 多模态',
      glmMultimodalDesc: '开启后，包含图片输入的 GLM 请求才会路由到此账号。纯文本 GLM 请求不受影响。',
